...
public class Scanner implements Iterator<Token>, Iterable<Token>
{
    ...
    public Token next()
    {
        ...
        if (character == '+') {
            ...
            boolean isDot = false; // prevent from more than one "dot" in number

            while ((this.character >= '0' && this.character <= '9') || this.character == '.') {
                if (isDot && this.character == '.') throw new UnexpectedCharacterException(this.character, this.locate(this.position));
                else if (this.character == '.') isDot = true;
                sb.append(String.valueOf(this.character));
                this.readChar();
            }
            token = this.makeToken(TokenType.NUM, sb.toString());
        } else if (character == '(') {
